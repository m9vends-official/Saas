...
export const listOrders = async (req, res, next) => {
    try {
        ...
        const {machine_id,
              assigned_machine_ids, status, page, limit} = req.query;

        let assigned_machine_ids = null;
        if (req.user.role === "TECHNICIAN") {
            const assignments = await MachineAssignment.find({ company_id: req.company_id, technician_id: req.user.user_id }).lean();
